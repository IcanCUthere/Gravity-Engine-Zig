...
pub const name: [*:0]const u8 = "editor";
...
fn load(scene: *flecs.world_t) callconv(.c) bool {
    ...
    const viewport = flecs.get(_scene, graphics.mainViewport, graphics.Viewport).?;
    ...
    guiDescriptorPool = gfx.CreateDescriptorPool(
    ...
    const vertexCode = graphics.shaders.getOrAdd("resources/shaders/id/id.vert") catch return false;
    vertexModule = gfx.CreateShaderModule(
    ...
    const fragmentCode = graphics.shaders.getOrAdd("resources/shaders/id/id.frag") catch return false;
    fragmentModule = gfx.CreateShaderModule(
        &gfx.ShaderModuleCreateInfo{
    ...
    renderPass = createRenderPass() catch return false;
    pipelineLayout = createPipelineLayout() catch return false;
    idPipeline = gfx.createPipeline(
        null,
        pipelineLayout,
        renderPass,
        vertexModule,
    ...
    createReadBackData(
        graphics.InputState.viewportX,
        graphics.InputState.viewportY,
    ) catch return false;
    ...
    const gizmo = graphics.Model.new(
        "Gizmo",
...
fn unload() callconv(.c) bool {
    ...
    inline for (components) |comp| {
        util.module.cleanUpComponent(comp, _scene) catch return false;
    }

    gui.backend.deinit();
    gui.deinit();

    gfx.DestroyDescriptorPool(guiDescriptorPool) catch return false;

    gfx.DestroyRenderPass(renderPass) catch return false;

    destroyReadBackData() catch return false;

    gfx.DestroyPipeline(idPipeline) catch return false;
    gfx.DestroyPipelineLayout(pipelineLayout) catch return false;
    gfx.DestroyDescriptorSetLayout(instanceTransformLayout) catch return false;
    gfx.DestroyShaderModule(vertexModule) catch return false;
    gfx.DestroyShaderModule(fragmentModule) catch return false;
...
pub fn onEvent(it: *flecs.iter_t) !void {
    ...
    const viewport = flecs.get(it.world, graphics.mainViewport, graphics.Viewport).?;
